...
//! with a live, cheap completion), choosing the agent roster (default 3-agent
//! roster or a custom comma-separated model list), and the number of review
...
///   3-agent roster and its synthesizer are retained.
...
pub async fn run_onboarding(save_path: &Path) -> Result<Config> {
    ...
    // 3. Roster: default 3-agent or custom.
    let use_default = Confirm::new()
        .with_prompt("Use the default 3-agent roster (Gemini, Grok, DeepSeek)?")
        .default(true)
        .interact()
        .map_err(prompt_err)?;
