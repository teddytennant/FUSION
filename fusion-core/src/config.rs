...
pub struct AgentConfig {
    ...
    pub name: String,
    /// Primary OpenRouter model id (e.g. `anthropic/claude-opus-4.8`).
    pub model: String,
...
impl Default for Config {
    /// The built-in default roster: the recommended two-model fusion of
    /// Claude Opus 4.8 and GLM 5.2. Two strong, independent models debating each
    /// other generally match or beat a larger roster; add a third (e.g. GPT 5.5)
    /// via config when you want it.
    fn default() -> Self {
        Config {
            api_key: None,
            agents: vec![
                AgentConfig {
                    ...
                    fallback_models: vec!["anthropic/claude-sonnet-4.6".into()],
                },
                AgentConfig {
                    name: "GLM 5.2".into(),
                    model: "z-ai/glm-5.2".into(),
                    role: Some("factual accuracy and critique".into()),
                    fallback_models: vec!["z-ai/glm-4.6".into()],
                },
            ],
            synthesizer: SynthesizerConfig {
                name: "Synthesizer (Claude Opus 4.8)".into(),
                model: "anthropic/claude-opus-4.8".into(),
                fallback_models: vec!["z-ai/glm-5.2".into()],
            },
            rounds: 2,
            max_tokens: 1000,
            temperature: 0.7,
